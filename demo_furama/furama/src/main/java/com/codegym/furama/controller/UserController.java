...
public class UserController {
    ...
    public String showList(Model model,
                           @RequestParam(required = false, defaultValue = "") String searchUsername,
                           @RequestParam Optional<Integer> page) {
        int pageBegin = 0;
        if (page.isPresent()) {
            pageBegin = page.get();
        }
        Pageable pageable = PageRequest.of(pageBegin, 4, Sort.by("username").descending());
        Page<User> userPage = userService.searchByName(searchUsername,pageable);
